...
auto ShowMinimumInRange(std::shared_ptr<std::vector<int>> numbers, const size_t start, const size_t end) -> void {
...
auto GenerateNumbers(const size_t size) -> std::shared_ptr<std::vector<int>> {
    auto numbers = std::make_shared<std::vector<int>>(size);
